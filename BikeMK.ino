...
byte valueCurPos = 0;                                             // позиция разряда длины колеса. Используется при более удобной настройки
...
void setup() {
  ...
  Serial.begin(19200);                                                 // настраиваем сериал
  ...
  if (!isSleep) {
    if (isMovement) {                                                     // если мы движемся
      ...
      if (millis() - lastCycleTurnTime > maxTimeIntrvl) {                 // значение интервала для сброса при скорости ниже 3км/ч
        ...
        startScan = false;
        writeDataToRadio();                                               // отправляем запрос пульсометру о конце считвания
      }
      ...
      if (isSendDynData) {
        if (millis() - sendDynDataTimeStamp >= 1000) {
          sendDynData();
        }
      }
    } else {
    ...
    if (saveSdData) {
      ...
      } else if (millis() - saveSdStarTmStmp >= 60000) {
        writeDynDataToSD();
        saveSdData = false;
        saveSdStarTmStmp = 0;
      }
    ...
    if (millis() - lifeCycleTime >= 600) {                                // если прошло больше чем 1 секунда  то обновить значения
      ...
      if (redrawValues) {                                                 // требуется обновить динамические параметры
        ...
        redrawValues = false;
        // или в меню отображения данных в меню с часами(для обновления часов)
        // требуется обновить экран по времени для мерцания значения часов
        if (menuPosition == 1)
